...
impl defmt::Format for Reg0 {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Reg0 {{ pwd_zcd: {=bool:?}, disable_auto_clk_switch: {=bool:?}, sel_clk: {=bool:?}, pwd_osc_int: {=bool:?}, pwd_cur_sns_cmp: {=bool:?}, cur_sns_thrsh: {:?}, pwd_overcur_det: {=bool:?}, overcur_trig_adj: {:?}, pwd_cmp_batt_det: {=bool:?}, en_lp_overload_sns: {=bool:?}, pwd_high_volt_det: {=bool:?}, lp_overload_thrsh: {:?}, lp_overload_freq_sel: {=bool:?}, lp_high_hys: {=bool:?}, pwd_cmp_offset: {=bool:?}, xtalok_disable: {=bool:?}, current_alert_reset: {=bool:?}, xtal_24m_ok: {=bool:?}, sts_dc_ok: {=bool:?} }}" , self . pwd_zcd () , self . disable_auto_clk_switch () , self . sel_clk () , self . pwd_osc_int () , self . pwd_cur_sns_cmp () , self . cur_sns_thrsh () , self . pwd_overcur_det () , self . overcur_trig_adj () , self . pwd_cmp_batt_det () , self . en_lp_overload_sns () , self . pwd_high_volt_det () , self . lp_overload_thrsh () , self . lp_overload_freq_sel () , self . lp_high_hys () , self . pwd_cmp_offset () , self . xtalok_disable () , self . current_alert_reset () , self . xtal_24m_ok () , self . sts_dc_ok ())
    }
}
#[doc = "DCDC Register 1"]
#[repr(transparent)]
...
impl defmt::Format for Reg1 {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Reg1 {{ reg_fbk_sel: {:?}, reg_rload_sw: {=bool:?}, lp_cmp_isrc_sel: {:?}, loopctrl_hst_thresh: {=bool:?}, loopctrl_en_hyst: {=bool:?}, vbg_trim: {=u8:?} }}" , self . reg_fbk_sel () , self . reg_rload_sw () , self . lp_cmp_isrc_sel () , self . loopctrl_hst_thresh () , self . loopctrl_en_hyst () , self . vbg_trim ())
    }
}
#[doc = "DCDC Register 2"]
#[repr(transparent)]
...
impl defmt::Format for Reg2 {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Reg2 {{ loopctrl_dc_ff: {=u8:?}, loopctrl_en_rcscale: {=u8:?}, loopctrl_rcscale_thrsh: {=bool:?}, loopctrl_hyst_sign: {=bool:?}, disable_pulse_skip: {=bool:?}, dcm_set_ctrl: {=bool:?} }}" , self . loopctrl_dc_ff () , self . loopctrl_en_rcscale () , self . loopctrl_rcscale_thrsh () , self . loopctrl_hyst_sign () , self . disable_pulse_skip () , self . dcm_set_ctrl ())
    }
}
#[doc = "DCDC Register 3"]
#[repr(transparent)]
...
impl defmt::Format for Reg3 {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Reg3 {{ trg: {=u8:?}, target_lp: {:?}, minpwr_dc_halfclk: {=bool:?}, disable_step: {=bool:?} }}" , self . trg () , self . target_lp () , self . minpwr_dc_halfclk () , self . disable_step ())
    }
}
