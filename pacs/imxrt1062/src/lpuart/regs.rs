...
impl defmt::Format for Baud {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Baud {{ sbr: {=u16:?}, sbns: {=bool:?}, rxedgie: {=bool:?}, lbkdie: {=bool:?}, resyncdis: {=bool:?}, bothedge: {=bool:?}, matcfg: {:?}, rdmae: {=bool:?}, tdmae: {=bool:?}, osr: {:?}, m10: {=bool:?}, maen2: {=bool:?}, maen1: {=bool:?} }}" , self . sbr () , self . sbns () , self . rxedgie () , self . lbkdie () , self . resyncdis () , self . bothedge () , self . matcfg () , self . rdmae () , self . tdmae () , self . osr () , self . m10 () , self . maen2 () , self . maen1 ())
    }
}
...
impl defmt::Format for Data {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Data {{ r0t0: {=bool:?}, r1t1: {=bool:?}, r2t2: {=bool:?}, r3t3: {=bool:?}, r4t4: {=bool:?}, r5t5: {=bool:?}, r6t6: {=bool:?}, r7t7: {=bool:?}, r8t8: {=bool:?}, r9t9: {=bool:?}, idline: {=bool:?}, rxempt: {=bool:?}, fretsc: {=bool:?}, paritye: {=bool:?}, noisy: {=bool:?} }}" , self . r0t0 () , self . r1t1 () , self . r2t2 () , self . r3t3 () , self . r4t4 () , self . r5t5 () , self . r6t6 () , self . r7t7 () , self . r8t8 () , self . r9t9 () , self . idline () , self . rxempt () , self . fretsc () , self . paritye () , self . noisy ())
    }
}
...
impl defmt::Format for Fifo {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Fifo {{ rxfifosize: {:?}, rxfe: {=bool:?}, txfifosize: {:?}, txfe: {=bool:?}, rxufe: {=bool:?}, txofe: {=bool:?}, rxiden: {:?}, rxflush: {=bool:?}, txflush: {=bool:?}, rxuf: {=bool:?}, txof: {=bool:?}, rxempt: {=bool:?}, txempt: {=bool:?} }}" , self . rxfifosize () , self . rxfe () , self . txfifosize () , self . txfe () , self . rxufe () , self . txofe () , self . rxiden () , self . rxflush () , self . txflush () , self . rxuf () , self . txof () , self . rxempt () , self . txempt ())
    }
}
...
impl defmt::Format for Modir {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Modir {{ txctse: {=bool:?}, txrtse: {=bool:?}, txrtspol: {=bool:?}, rxrtse: {=bool:?}, txctsc: {=bool:?}, txctssrc: {=bool:?}, rtswater: {=u8:?}, tnp: {:?}, iren: {=bool:?} }}" , self . txctse () , self . txrtse () , self . txrtspol () , self . rxrtse () , self . txctsc () , self . txctssrc () , self . rtswater () , self . tnp () , self . iren ())
    }
}
...
impl defmt::Format for Stat {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Stat {{ ma2f: {=bool:?}, ma1f: {=bool:?}, pf: {=bool:?}, fe: {=bool:?}, nf: {=bool:?}, or: {=bool:?}, idle: {=bool:?}, rdrf: {=bool:?}, tc: {=bool:?}, tdre: {=bool:?}, raf: {=bool:?}, lbkde: {=bool:?}, brk13: {=bool:?}, rwuid: {=bool:?}, rxinv: {=bool:?}, msbf: {=bool:?}, rxedgif: {=bool:?}, lbkdif: {=bool:?} }}" , self . ma2f () , self . ma1f () , self . pf () , self . fe () , self . nf () , self . or () , self . idle () , self . rdrf () , self . tc () , self . tdre () , self . raf () , self . lbkde () , self . brk13 () , self . rwuid () , self . rxinv () , self . msbf () , self . rxedgif () , self . lbkdif ())
    }
}
