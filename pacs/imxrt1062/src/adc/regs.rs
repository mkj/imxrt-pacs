...
impl defmt::Format for Cfg {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Cfg {{ adiclk: {:?}, mode: {:?}, adlsmp: {=bool:?}, adiv: {:?}, adlpc: {=bool:?}, adsts: {:?}, adhsc: {=bool:?}, refsel: {:?}, adtrg: {=bool:?}, avgs: {:?}, ovwren: {=bool:?} }}" , self . adiclk () , self . mode () , self . adlsmp () , self . adiv () , self . adlpc () , self . adsts () , self . adhsc () , self . refsel () , self . adtrg () , self . avgs () , self . ovwren ())
    }
}
...
impl defmt::Format for Gc {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Gc {{ adacken: {=bool:?}, dmaen: {=bool:?}, acren: {=bool:?}, acfgt: {=bool:?}, acfe: {=bool:?}, avge: {=bool:?}, adco: {=bool:?}, cal: {=bool:?} }}" , self . adacken () , self . dmaen () , self . acren () , self . acfgt () , self . acfe () , self . avge () , self . adco () , self . cal ())
    }
}
...
impl defmt::Format for Hs {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Hs {{ coco0: {=bool:?}, coco1: {=bool:?}, coco2: {=bool:?}, coco3: {=bool:?}, coco4: {=bool:?}, coco5: {=bool:?}, coco6: {=bool:?}, coco7: {=bool:?} }}" , self . coco0 () , self . coco1 () , self . coco2 () , self . coco3 () , self . coco4 () , self . coco5 () , self . coco6 () , self . coco7 ())
    }
}
